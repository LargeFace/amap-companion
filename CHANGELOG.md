...
## 2026-05-05 新旧 UI 切换与仪表盘新 UI 优化

- 主页新增悬浮窗样式切换：默认使用旧 UI，并可手动切换到“新 UI（测试中）”。
- 新 UI 与旧 UI 现在可以真正切换，切换后会同时重建主悬浮窗和仪表盘镜像，避免只更换背景、不更换布局结构。
- 修复新 UI 在退出导航或巡航后仍残留当前道路名称、下一路口和距离信息的问题。
- 新 UI 在待命状态下不再显示车头/道路等详细状态内容，避免出现无效信息残留。
- 仪表盘镜像中的新 UI 改为更紧凑的双列布局，并进一步缩小上下间距，减少被上下裁切的概率。
- 优化新 UI 中“下一路口”卡片的留白和距离布局，使道路名称、距离与提示文案之间更易读。
