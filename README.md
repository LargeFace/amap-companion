...
- 支持检查更新、展示更新详情，并通过 `pm install`、`adb install` 或系统安装器升级。
...
设置后可以在主界面点击“检查更新”。如果服务器返回的 `versionCode` 大于当前应用版本，应用会先展示版本号和更新日志。点击“更新”后可选择安装方式。

`pm install` 模式会尝试把 APK 写入：

```text
/data/local/tmp/amap_companion_update.apk
```

然后执行：

```text
pm install -r -d <APK路径>
```

`adb install` 模式同样会先写入上述路径，然后执行：

```text
adb install -r -d <APK路径>
```

系统安装器模式会调用 Android 系统安装界面。设备管理员模式入口已预留，授权和静默安装代码暂未实现。

由于部分车机系统限制普通应用静默安装，如果没有系统权限、root 权限、车机白名单授权或设备管理策略，安装步骤仍可能失败。失败时应用会显示命令输出，便于排查权限问题。
